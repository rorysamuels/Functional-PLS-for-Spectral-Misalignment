...
$$
\arg\max_{w(w)} \text{Cov}^2\left(y, \int_a^bx(w)r(w)dw\right), \quad \text{subject to:}
$$
...
$$
\mathbf{U}_{(ij)} = \int_a^bx_i(w)B_j(w)dt,\ \ i = 1,...,n,\ \ j = 1,...,M+d.
$$
...
$$
\boldsymbol{\Theta}_{(ij)} = \int_a^bB^x_i(w)B_j(w)dt,
$$
...
$$
y_i = \int_0^1x_i(w)\beta(w)dt + \epsilon_i
$$
...
- $\beta(w) = 10(w-1)^2 + 30\text{cos}(4\pi t^3)$
...
- Predict $y^{test}$ using observations of $x^{test}(w)$ on $G_B$ and $\hat{\beta}(w)$.
...
pmse_label_A <- glue("PMSE = {round(pmse(y_test,pls_preds_A),4)}")
pmse_label_B <- glue("PMSE = {round(pmse(y_test,pls_preds_B),4)}")
tb_ann <- tibble(x = c(-3.5,-3.5), y = c(4,4), label = c(pmse_label_A, pmse_label_B), grid = c("A","B"))
...
$$
y_i = \mathbf{U}\boldsymbol{\gamma} + \epsilon_i.
$$
