...
$$
\arg\max_{w(t)} \text{Cov}^2\left(y, \int_a^bx(w)r(w)dw\right), \quad \text{subject to:}
$$

$$
\text{Cov}\left(\int_a^bx(w)r_m(w)dt,\int_a^bx(w)r(w)dt\right) = 0\ \ \text{for}\ \ m = 1,...,k-1,\quad \text{and}
$$
...
Let $\mathbf{B}(t) = (B_1(t),...,B_{M+d}(t))'$ be a vector of $M+d$ B-spline basis functions of degree $d$ defined over $M-1$ equally spaced knots on $[a,b]$.
...
$$
\hat{\beta}_{FPLS}(t) = (\mathbf{R}\boldsymbol{\alpha})'\mathbf{B}(t)
$$
...
- In practice, we observe $p$ discrete points along each $x_i(t)$
...
where $\mathbf{B}^x(t)$ is a vector of $M_x + d$ B-spline basis functions and $\mathbf{c}_i$ is a vector of $M_x + d$ basis coefficients. If we define
