...
We applied the same method to the AOP Crown data-set to predict \texttt{d15N} from observed spectra. After joining the site trait data and spectra by \texttt{SampleSiteID}, and removing both "bad bands" and NA observations we had:
...
- Obtain PLS coefficients $\boldsymbol{\hat{\beta}}_A$ using $y^{train}$ and $x^{train}(w)$ on $G_A$
- Approximate $x^{test}(w)$ on $G_A$ using $x^{test}(w)$ on $G_B$ with straight lines
